...
- **Monitoring live**: Hermes cron jobs every 30 min — `LT n8n-postgres-runner monitor` (job `33eea46dd9b3`, checks n8n/postgres/runner, emails alerts) and `LT watch-email auto-fix` (job `b3f164cd1746`, auto-fixes runner-network class from Gmail alerts, escalates the rest). Email transport: Hermes Google OAuth Gmail (`edmundocadorniga@gmail.com`), tested (`SENT_OK id=1a07c62cb42810a4`).
