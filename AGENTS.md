...
- Live n8n: 70/75 workflows rotated via API (backs up pre-change JSON per workflow in `%LOCALAPPDATA%\Temp\lt_pit_rotation\`); re-verified by `scripts/inventory_n8n_pits.py` — only the new PIT remains in active workflows, all HTTP 200. Dead 401 token `13c5a02e54a2` gone from active workflows.
...
## ✅ IN PROGRESS: 2026-09-09 Executive Report — SDR Performance & Owner Attribution (Phases 1–2 + Phase 4 LIVE)
...
- **Remaining (Phase 3–5)**: monitor the daily 08:00 LA meeting-outcome digest runs (Slack `#sales`) and confirm SDRs start marking Showed/No-Show in GHL; get Cameron/Janvi sign-off on the confirmed per-SDR ranking scope wording (Booked + SQLs + MQL→SQL); confirm nightly Rollups/QA health after GA4/GSC reconnection. Full plan: `docs/sessions/2026-09-09-executive-report-sdr-attribution-plan.md`.
...
- **Next:** confirm the GHL booking workflow is saved/published with custom-data key `assignedSDR`, then observe the next real booking webhook and verify `sync.originatingSdr = stamped`, the contact field value, and the resulting SDR row in Exec Summary. Historical bookings remain unrecoverable.
...
The original event that called `pool.end()` remains unresolved. No restart, deployment, workflow operation, configuration write, or database change was performed during the investigation. A September 4 kernel soft-lockup involving `postgres` and `soketi-server` is a clue but not proven causal because the first retained pool error was September 7. The next session must correlate Docker/Coolify, PostgreSQL, n8n, runner, host, and network events around `2026-09-07 12:52 UTC`, inspect bounded n8n 2.37.9 pool shutdown/recovery paths, and—after explicit approval—restart only the n8n main service if readiness remains failed. Do not restart PostgreSQL/Redis or rotate `N8N_ENCRYPTION_KEY` casually.
...
- **n8n runner `pg` + network fix (2026-08-31)** — the Coolify-managed runner (`n8n-runner-n44wksswcocwk88ogcog8c48`, auth token `<N8N_RUNNERS_AUTH_TOKEN — see Coolify env>`) had `NODE_PATH=/opt/pg-node_modules` which does NOT work with the **pnpm** pg layout that `n8n/docker-compose.yml`'s `dockerfile_inline` produces (pg lives at `/opt/pg-node_modules/node_modules/pg`), so every pg-using Code node failed `Cannot find module 'pg'` (this is why Leads Ingest errored on 57 consecutive runs). Fixed `/etc/n8n-task-runners.json` + env in the running container AND in the repo compose: `n8n/docker-compose.yml` now bakes `NODE_PATH=/opt/pg-node_modules/node_modules` in both the `dockerfile_inline` `env-overrides` and the runner service `environment`. Also attached the runner to `coolify-shared` in the repository Compose design so it can resolve the `postgres` hostname (the compose declares `networks: coolify-shared:` for the runner service). However, the 2026-09-07 live Coolify-generated Compose inspection found configuration drift: the running generated service still has `extra_hosts: postgres:host-gateway` and is attached only to the private resource network, so the runner currently resolves `postgres` to stale `10.0.0.1` instead of PostgreSQL on `coolify-shared` at approximately `10.0.2.3`. Treat the network fix as OPEN until the authoritative Coolify source is corrected, the runner is recreated, and DNS/TCP/GHL-ingest verification passes. See `docs/sessions/2026-09-07-runner-network-path-verification.md`. After the next Coolify rebuild, verify: `docker exec <runner> sh -c 'node -e "require.resolve(\"pg\")"'` succeeds, `getent hosts postgres` resolves to the PostgreSQL container, and a pg-using workflow (e.g. Leads Ingest) runs green.
...
- Partnership LinkedIn reply recovery (2026-08-12): Campaign Channels now reports 3 verified Partnership replies. Jaret Christopher was already present; David Schachter (`rvWEW2K2WYeQ7v6zypDdZQ`, 2026-08-10) and Gretchen Gailey (`8UF3lxibUmKYaG87h1F5Pg`, 2026-08-06) were recovered from the Unipile API with their original timestamps and inserted idempotently into `linkedin_activity_events`. `LT - LinkedIn Unipile New Messages` (`7o5EBdvwAuIaWW7k`) is published on `f96dafba-9818-4aab-8656-c2e4e2ab8480` with a malformed form-payload fallback so unescaped Unipile JSON no longer loses critical inbound fields. **2026-09-10 fix**: GHL conversations 404/422 resolved - `Create LinkedIn Contact and Add Inbound Message` node in `LT - LinkedIn Unipile New Messages` (`7o5EBdvwAuIaWW7k`) changed `this.helpers.request(` to `this.helpers.httpRequest(`, `uri` to `url`, and corrected endpoint from `'/conversations/messages'` to `'/conversations/messages/inbound'` (the `/inbound` suffix is required by GHL for posting inbound messages). Published version `127504a5-08ae-41b4-ad5c-f2a39a068e82` (versionId == activeVersionId). Correct GHL API flow: POST `/oauth/locationToken` with `{companyId, locationId}` using `Authorization: Bearer <GHL_PIT>` to get `access_token`, then use that as Bearer for `POST /conversations/messages/inbound` with body `{"type":"Custom","contactId":"<ghl_contact_id>","message":"<text>","conversationProviderId":"6a58a14ff3023bea3783c152"}`.
