...
- **2026-07-15**: LinkedIn Unicode encoding fix applied across all 3 message-sending workflows (DM Sequence, Follower DM, Dispatcher). Added `sanitizeMessage()` function to replace smart quotes/apostrophes/dashes/ellipsis with ASCII equivalents before Unipile API calls. Fixes garbled text (e.g., `can't` → `canâ€™t`) caused by Unicode→Latin-1 decoding in the JSON.stringify → Unipile pipeline. Created `scripts/suppress_linkedin_dms.py` for one-command DM suppression.
