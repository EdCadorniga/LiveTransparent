...
Updated: 2026-07-09 (LinkedIn workflows paused for no-delay DM test; live sender guardrails added to block John-branded copy; connection accepted and delivery verified; follower DM template fix + John→Jason GHL workflow migration documented)
...
- **LinkedIn Follower DM Template Corruption Fix (2026-07-09)**: `LT - LinkedIn Follower DM Sequence (Unipile)` (`pq7XVajNFnnwMUTr`) had garbled multi-byte characters in `TEMPLATE_REGISTRY` (curly quotes, em-dashes, wave emoji mangled into garbage). Replaced with clean text via n8n REST API PUT. All 12 LinkedIn/Instagram/SMS workflows verified clean.
- **John→Jason Migration — Current GHL Status (2026-07-09)**: The repo HTML templates under `marketing/email-marketing/john-follow-up/source/` were renamed to Jason and updated to booking link `{{trigger_link.quqSUM8bckKaOIktVvgU}}`. The six matching live GHL email templates in folder `69e0c9069af5986541802d88` were also updated via API, including template `01` metadata `fromName = Jason from Transparent eCom` and `fromEmail = jason@livetransparent.com`. The owner then manually updated all `Send email` actions in `JohnFollowup Emails and SMS` (`f6b44e34`), but agent-side verification of the workflow graph was blocked because GHL workflow builder endpoints still return 404 for this workflow type and the available browser profiles were not authenticated. The remaining likely John-visible GHL items are: `Task John to create a LinkedIn Connection Request when MQL is found` (`25cd82a2`), `Simpletexting Send SMS for failed sends - John SMS1` (`41c6aecd`), and `Simpletexting Send SMS for failed sends - John SMS2` (`a99f96d9`).
- **LinkedIn pause + no-delay DM test (2026-07-09)**: All active production LinkedIn `n8n` workflows were manually deactivated for a controlled test: `Old7ZvyVYgFaJgDr`, `7o5EBdvwAuIaWW7k`, `pq7XVajNFnnwMUTr`, `3ttEvr5NMcQCS4Hp`, `ceaKnz6E3onQrZpt`, `d0tEtijajisIsYcs`, `Zt8p2aYtIuY0HK18`, `fXxw5lanZcDmUrst`. Manual test workflow `LT - LinkedIn DM Sequence Test (No Delay)` (`wnpVYUNFLyNe5cS6`) was created, targeting `https://www.linkedin.com/in/edmundo-c-a06372166/` and sending the 4 LinkedIn DMs immediately (`interMessageDelayMs = 0`).
- **LinkedIn connection acceptance verified (2026-07-09)**: Cameron Karkut's Unipile account initially showed Edmundo's profile as `invitation.status = PENDING`, `is_relationship = false`. After the invite was accepted in LinkedIn, Unipile reported `is_relationship = true` and `network_distance = FIRST_DEGREE`. Owner confirmed the DM messages looked correct from the recipient side.
- **LinkedIn invite copy audit (2026-07-09)**: Live `n8n` invite defaults already say `Transparent eCom` in the dispatcher, internal test webhook, and `social_outreach_templates.ts`. If `LiveTransparent` appears again in a future invite, check GHL-side `body.message` overrides first.
- **LinkedIn outbound guardrails (2026-07-09)**: Live sender workflows `fXxw5lanZcDmUrst` and `pq7XVajNFnnwMUTr` now reject John-branded outbound copy before calling Unipile. The paused internal test invite workflow `Zt8p2aYtIuY0HK18` and the main DM workflow `d0tEtijajisIsYcs` still need a full-body rewrite if they are ever reactivated.
- **Next-session priority (2026-07-09 handoff)**: Start by finding the exact GHL-side source of the old John-branded LinkedIn connection request. Primary suspect: `Task John to create a LinkedIn Connection Request when MQL is found` (`25cd82a2`), whose HTTP POST sends `body.message = "Hey {first_name} - quick connect. John here with Transparent eCom."` to the n8n connection-request webhook and can override the Cameron default. Reproduce the trigger path and confirm whether this workflow is the actual live source. Only after that should time go to the two failed-send SMS GHL workflows (`41c6aecd`, `a99f96d9`) if they still show John in recipient-facing SMS copy.
...
- Decide whether to reactivate the paused LinkedIn production workflows after the successful no-delay DM test. [PENDING — owner confirmation]
- Find and fix the GHL-side source of the old John-branded LinkedIn connection request override, then clean up any remaining John-visible failed-send SMS workflows. [NEXT SESSION PRIORITY]
...
- As of 2026-07-09, all production LinkedIn `n8n` workflows are intentionally paused. Do not describe the LinkedIn pipeline as active again until those workflows are explicitly reactivated and live state is re-checked.
- For the next session, the main unresolved LinkedIn question is not delivery or connection state; it is the GHL-side source of the old John-branded connection-request copy.
