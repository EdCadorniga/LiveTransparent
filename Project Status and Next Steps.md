...
Updated: 2026-07-09 (LinkedIn Follower DM template corruption fix + John→Jason GHL workflow migration documented)
...
- **John→Jason Migration — Remaining GHL Workflows (2026-07-09)**: 4 GHL workflows still reference John and need manual editing in GHL UI: `Task John to create a LinkedIn Connection Request when MQL is found` (`25cd82a2`), `JohnFollowup Emails and SMS` (`f6b44e34`), `Simpletexting Send SMS for failed sends - John SMS1` (`41c6aecd`), `Simpletexting Send SMS for failed sends - John SMS2` (`a99f96d9`). The n8n Connection Request webhook (`Zt8p2aYtIuY0HK18`) accepts `body.message` override — the GHL workflow sends `"Hey {first_name} - quick connect. John here with Transparent eCom."` which overrides the Cameron default. GHL REST API returns 404 for workflow builder/actions endpoints on these types; must edit in GHL UI. See AGENTS.md for full details and URLs.
