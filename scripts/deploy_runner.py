...
if not POSTGRES_PASSWORD:
    raise SystemExit("Set POSTGRES_PASSWORD before deploying the runner")
...
if not RUNNER_AUTH_TOKEN:
    token_command = (
        "docker inspect n8n-runner --format '{{range .Config.Env}}{{println .}}{{end}}' "
        "| sed -n 's/^N8N_RUNNERS_AUTH_TOKEN=//p'"
    )
    RUNNER_AUTH_TOKEN = client.exec_command(token_command)[1].read().decode().strip()
    if not RUNNER_AUTH_TOKEN:
        raise SystemExit("Set N8N_RUNNERS_AUTH_TOKEN or keep an existing n8n-runner container to reuse its token")
...
for command in commands:
    ...
    display_command = command.replace(RUNNER_AUTH_TOKEN, "<redacted>").replace(POSTGRES_PASSWORD, "<redacted>")
