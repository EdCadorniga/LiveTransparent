...
- Current automated booked handoff is for `Regulated Ads On Social/Search`, `Book a demo`, and `Book 1:1 with Cameron` calendars.
...
- `Referral`
- Booking only when calendar is one of the MQL-qualified calendars:
...
- MQL-calendar bookings should also receive tag `SQL`.
- MQL-calendar bookings should end in `Sales -> Discovery Scheduled`.
- Exception: `Book 1:1 with Cameron` is MQL-only (no `SQL`, no Sales handoff).

## Booking Slack Rule
- `#leads` booking alerts are sent from the filtered GHL booking automation.
- Only MQL-qualified calendar bookings (`Regulated Ads On Social/Search`, `Book a demo`, `Book 1:1 with Cameron`) should post to Slack.
