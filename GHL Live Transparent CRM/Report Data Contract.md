...
- SDR assignment source, owner-alignment result, conflict count, and unassigned Sales Outreach records
  - **Status (2026-09-09, Phases 1–2 + Phase 4 implemented):** owner/assigned data captured in the ingests (opportunities `dimensions_json->>'assigned_to'`, appointments `assigned_user_id`+`contact_id`, contacts `payload_json`). `report_sdr_registry` maps user IDs → names; Exec Summary returns per-SDR `sdrPerformance` (booked / showed / no-show / cancelled / SQLs created / MQL→SQL / won / lost / revenue) with `meetingsBooked` aligned to appointments (`basis appointments_start_at`), plus `leadSourceBreakdown`/`leadSourceCoverage` for MQL/SQL lead-source attribution. Remaining: GHL appointment-status update (Phase 3), unknown-owner-ID identification. See `docs/sessions/2026-09-09-executive-report-sdr-attribution-phase1-2.md` and `docs/sessions/2026-09-09-executive-report-sdr-attribution-phase4-lead-source.md`.
