...
- **n8n runner `pg` + network fix (2026-08-31)** — the Coolify-managed runner (`n8n-runner-n44wksswcocwk88ogcog8c48`, auth token `<N8N_RUNNERS_AUTH_TOKEN — see Coolify env>`) had `NODE_PATH=/opt/pg-node_modules` which does NOT work with the **pnpm** pg layout that `n8n/docker-compose.yml`'s `dockerfile_inline` produces (pg lives at `/opt/pg-node_modules/node_modules/pg`), so every pg-using Code node failed `Cannot find module 'pg'` (this is why Leads Ingest errored on 57 consecutive runs). Fixed `/etc/n8n-task-runners.json` + env in the running container AND in the repo compose: `n8n/docker-compose.yml` now bakes `NODE_PATH=/opt/pg-node_modules/node_modules` in both the `dockerfile_inline` `env-overrides` and the runner service `environment`. Also attached the runner to `coolify-shared` so it resolves the `postgres` hostname (it was only on the app-private network) — the compose already declares `networks: coolify-shared:` for the runner service. **There are two independent runner build paths with different pg layouts — keep them straight:** (1) **Coolify/compose** (`n8n/docker-compose.yml`, pnpm) → `NODE_PATH=/opt/pg-node_modules/node_modules`; (2) **`scripts/deploy_runner.py`** + `n8n/runners/Dockerfile` + `n8n/runners/n8n-task-runners.json` (npm layout, custom `n8n-runner`) → `NODE_PATH=/opt/pg-node_modules`. Do not cross them. After the next Coolify rebuild, verify: `docker exec <runner> sh -c 'node -e "require.resolve(\"pg\")"'` succeeds, `getent hosts postgres` resolves, and a pg-using workflow (e.g. Leads Ingest) runs green.
...
- Architecture: the dashboard is a static HTML and JavaScript SPA at reports.livetransparent.com. It calls the report-host proxies at `/api/report/executive/summary`, `/api/report/executive/campaign-channels`, and `/api/report/executive/outgoing-calls`; all render client-side. The public host serves build `2026-08-17-v26-social-reporting-accuracy` with campaign filters, LinkedIn and Instagram ledger metrics, comparison view, opportunity attribution, SMS diagnostics, explicit Social Planner placement/account-statistics coverage, outgoing-call detail, resolved GHL stage names, and responsive wide-table containment.
