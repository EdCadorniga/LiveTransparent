...
- Campaign Channels: This table is the cross-channel campaign view. It shows separate named rows for DAN, Emerald, Partnership, SMS, Vapi Brand, and Vapi Dispensary, with channel-specific sends, engagement, replies, DMs, calls, qualification, booked metrics, and selected-window opportunity counts where source data exists. Use the campaign and channel filters to focus on one segment.
- Campaign comparison: Switch to Compare to review active campaign totals side by side, including period-over-period changes. Expand a campaign row for its channel-specific funnel details.
- SMS Delivery Summary: This panel separates sent, delivery-failed, reply, and normalized provider failure reasons. Treat provider failures and duplicate-send failures as delivery diagnostics, not as replies or unsubscribes.
...
- Architecture: the dashboard is a static HTML and JavaScript SPA at reports.livetransparent.com. It calls the report-host proxies at `/api/report/executive/summary`, `/api/report/executive/campaign-channels`, and `/api/report/executive/outgoing-calls`; all render client-side. The public host serves build `2026-08-12-v23-mobile-overflow` with campaign filters, comparison view, opportunity attribution, SMS diagnostics, social fields, outgoing-call detail, resolved GHL stage names, and responsive wide-table containment.
...
- Campaign response shape: the campaign summary returns `window`, `comparison`, `campaignChannelBreakdown`, and SMS diagnostic fields. Rows contain `channel`, `campaign`, selected/prior metrics, email metrics and rates, LinkedIn invite/acceptance/DM/reply metrics, Vapi outcome metrics, and selected-window opportunity counts. DAN attribution uses release logs; Emerald uses bucket/enrollment data; Partnership uses `partnership_release_log` and partnership LinkedIn state; SMS uses `campaign_key`; LinkedIn uses activity events and source routing; and Vapi uses queue campaign IDs. Dynamic source campaigns remain visible as additional rows.
...
- Attribution note: campaign opportunity counts are based on current campaign tags plus opportunity creation dates. They are not immutable historical attribution, so tag changes can affect future report results.
- Native GHL report note: report `6a67dce4a51a4360c60963a3` is a complementary CRM operational report saved to `Last 30 days`; it does not replace the cross-channel Executive Report.
...
- **SimpleTexting next step**: production schedules were temporarily paused during the n8n execution-dispatcher incident. After the next Coolify redeploy, verify `N8N_RUNNERS_TASK_REQUEST_TIMEOUT=300`, webhook URL/deprecation settings, readiness, PostgreSQL connections, and zero stale scheduled work. Then follow the one-by-one sequence in `docs/n8n-stale-execution-recovery.md`: Step Runner, Phone Backfill, Warmup dry-run, Pool dry-run/small batch, and one approved live provider test. Keep State Diagnostic, Send Count Check, the legacy staged inbound webhook, and Campaign Sequencer inactive until their roles are explicitly approved.
...
Updated: 2026-08-12 (final recovery handoff and severity-ranked continuation order)
...
- GHL leads ingest is active hourly on published version `d29b7af9-0b69-4fc7-a53c-c23dd24b0825`. Execution `742843` wrote 500 distinct contacts and matching sync-run, watermark, and source-health records. Opportunity ingest and voice queue/attempt coverage still need separate verification.
- Executive Report build `2026-08-12-v23-mobile-overflow` is deployed. Pipeline/stage charts now resolve live GHL IDs to names in Current Open Deals, Active Deals, stage movement, velocity, and pipeline overview. The map was reconciled against the official GHL pipeline API, including Warm Vapi and Partnership stages. Desktop and 390px mobile checks found no raw stage IDs or page-level horizontal overflow; wide tables scroll within their panels.

For the complete recovery narrative and continuation order, read [`docs/handoff/2026-08-12-report-recovery.md`](docs/handoff/2026-08-12-report-recovery.md).

### Severity-Ranked Open Work

1. **Critical: repair and prove GHL Sales Ingest** (`aYT5oHcgmBALzHy5`). Last inspected execution `742754` failed `Fetch Opportunities` with HTTP `401`; both Postgres v2.6 write nodes must become one atomic direct-`pg` transaction. This is the next agent's first implementation task.
2. **Critical: restore source coverage with provenance**. After Sales Ingest succeeds, recover opportunities, voice attempts/outcomes, email/release logs, main LinkedIn state, and SimpleTexting state one source at a time. Do not fabricate historical data.
3. **Critical: authenticate public write boundaries and keep outbound gated**. Review Call Outcome Ingest, Warm intake, and SimpleTexting send authentication. Do not manually execute live dialer/sender workflows without approval.
4. **High: audit then backfill `ghl_contact_id`**. The checkpoint was `0/13,868`; inspect audit results and ambiguity before mutation.
5. **High: verify voice persistence safely**. Prove release-lock, attempt, and callback writes with non-sending branches where possible.
6. **High: migrate embedded secrets** to credentialed HTTP Request nodes or approved protected runtime configuration, then rotate exposed values.
7. **Medium: fix report correctness debt**. Remove duplicate Executive Summary JSON keys, fix request timezone drift, and add selected-period filters to `stageVelocity`, `sql_contacts`, and `pool_distribution` after source recovery.
8. **Medium: add OAuth-backed social statistics** for reach, impressions, and saves; complete approved native GHL report widgets/page names through the UI.
9. **Low: clean legacy nodes/scripts and stale historical prose** only after live paths are stable.
...
Measured directly on live Postgres after execution `742843`:

| Source | Current Rows |
|--------|-------------:|
| `report_raw_ghl_contacts` | 500 |
| `report_raw_ghl_opportunities` | 0 |
| `voice_call_queue` | 1 pending |
...
| `emerging_pool_contacts` / with GHL ID | 13,868 / 0 |
...
- **Executive Report response-rate + social-engagement fixes (2026-08-04)**: User reported that 1 LinkedIn partnership response and 1 email partnership response showed as 0 response rate, and that LinkedIn/email data for the 3 campaigns (New attribution model - brands, New attribution model - dispensaries, Partnerships) looked wrong. Root-cause investigation found 4 bugs, all fixed and published:
  ...
  6. **Social metric fields expanded**: Executive Summary API and deployed report build `2026-08-08-v18-opportunity-attribution` expose likes, comments, shares, saves, reach, and impressions when present in the source payload. Current raw post ingest still reports 24 likes, 3 comments, 4 shares, and zero saves/reach/impressions.
...
- **SimpleTexting workflow hardening (2026-08-06)**: bootstrapped `SimpleTexting_Campaign_State` and `SimpleTexting_Campaign_Event_Log` plus due-send, phone-lookup, lock, and event-log indexes; removed runtime DDL; split Warmup into preparation and send tasks; removed unsafe HTTP wrapper patterns; changed Step Runner to every 5 minutes and Phone Backfill to every 10 minutes; added advisory/row-claim overlap guards. All five production schedules remain unpublished pending one-by-one validation. See [docs/n8n-stale-execution-recovery.md](docs/n8n-stale-execution-recovery.md).
...
- **SMS campaign**: SimpleTexting account re-enablement is active after staged smoke tests. Step Runner (`dUyOfxllvkxZavaw`), Phone Backfill (`8hQKQi1PooYDFxNR`), Warmup Dispatcher (`dZQLlbTLkpE1843X`), and Pool Dispatcher (`usxYXSuc4ahw40V3`) are active and published. Smoke executions succeeded: Step Runner `721562`, Phone Backfill `721377`, Warmup `721541`, and Pool `721576`; Warmup had zero candidates/sends and Pool had zero eligible contacts. Step Runner explicitly sends `dryRun=false`; Warmup, Pool, and the send boundary have `defaultDryRun=false`. Campaign Sequencer (`7mSiivR3NhtLIcNz`) remains unpublished to avoid a duplicate sender path. State Diagnostic, Send Count Check, and the legacy staged inbound webhook remain inactive by design.
- **SimpleTexting remediation**: Provider account re-enablement and the first staged workflow smoke tests are complete. Monitor the four active paths after the timeout-configured redeploy; keep Campaign Sequencer unpublished.
...
1. **Repair GHL Daily Sales Ingest first**: workflow `aYT5oHcgmBALzHy5`, last inspected active version `4f3e8068-8864-4b4d-9286-ba4d618cc3a8`, failure `742754`. Replace stale auth, migrate both writes to one atomic direct-`pg` transaction, publish, run, and verify opportunity/history/sync/watermark/health rows.
2. **Restore source coverage one system at a time** with supported ingest/replay and provenance.
3. **Authenticate public write boundaries** before broader outbound activity; retain explicit approval gates for manual sends/calls.
4. **Audit and backfill `emerging_pool_contacts.ghl_contact_id`** only after healthy ingestion.
...
- **Campaign-level reporting dimensions**: `LT - Report Campaign Channel Summary` (`MvPLbUAN9IIQikxb`) returns separate DAN, Emerald, Partnership, Vapi Brand, Vapi Dispensary, and SMS rows. The public report build `2026-08-08-v18-opportunity-attribution` has campaign/channel filters, Vapi filters, drill-downs, comparison view, campaign opportunity counts, SMS diagnostics, and LinkedIn invite/accept/reply columns. Reply instrumentation is live and the two known historical replies are backfilled. The remaining reporting gap is OAuth-backed social statistics ingestion; native GHL campaign-tag and email-detail widgets remain open.
...
| LT - SimpleTexting SMS Send (Webhook, Staged) | Q3Ivnwe4z2Y3cD7A | Active | Mirrors successful outbound campaign sends into GHL Conversations under `SimpleTexting SMS`. |
| LT - SMS Idempotent Send | gwaEpWDpTIwsafi8 | Active | Canonical deduplicated SMS boundary. Called by outbound router and campaign send paths. |
| LT - SimpleTexting Warmup Dispatcher (Staged) | dZQLlbTLkpE1843X | Unpublished | Split preparation/send graph; `defaultDryRun=false`, bounded live test required. |
| LT - SimpleTexting Campaign Step Runner | dUyOfxllvkxZavaw | Unpublished | Every 5 minutes; atomic claims and overlap guard. |
| LT - SimpleTexting Campaign Phone Backfill | 8hQKQi1PooYDFxNR | Unpublished | Every 10 minutes; row claims and overlap guard. |
...
3. Repair GHL Sales Ingest (`aYT5oHcgmBALzHy5`) from failure `742754` before any backfill or report interpretation.
4. Verify the repaired workflow's published version and database writes.
5. Audit `ghl_contact_id` candidate matches, then restore voice/campaign/source state one system at a time.
