...
- Campaign summary active version `14caff0f-fa99-4bab-9867-792ef8836de6` returns named channel/campaign rows. DAN uses release-log campaign fields, Emerald uses bucket/enrollment data, SMS uses `SimpleTexting_Campaign_Event_Log.campaign_key`, LinkedIn uses `linkedin_activity_events` joined to `emerging_pool_contacts.source_list`, and Vapi uses queue campaign IDs. Catalog rows for `General outbound`, `Partnership emails`, `xyz`, and `abc` remain zero until matching source events exist; `Partnership LinkedIn` is a zero-safe catalog row.
...
- **Campaign-level reporting dimensions**: `LT - Report Campaign Channel Summary` (`MvPLbUAN9IIQikxb`, active version `14caff0f-fa99-4bab-9867-792ef8836de6`) returns named campaign/channel rows. DAN uses release-log campaign fields, Emerald uses bucket/enrollment data, SMS uses `SimpleTexting_Campaign_Event_Log.campaign_key`, LinkedIn uses `linkedin_activity_events` joined to `emerging_pool_contacts.source_list`, and Vapi uses queue campaign IDs. `General outbound`, `Partnership emails`, `xyz`, and `abc` are present as catalog rows; `Partnership LinkedIn` is now a zero-safe catalog row. The public report build `2026-07-31-v11-campaign-breakdown` has All, Email, LinkedIn, SMS, and VAPI campaign filters. Emerald event attribution still has an `Emerald - attributed` fallback. LinkedIn activity is currently zero because `linkedin_activity_events` has no stored events; sender instrumentation remains a follow-up before LinkedIn campaign counts can become nonzero.
...
GA4 and GHL ingestion are live. GSC ingestion remains blocked until the `GSC - Cameron Livetransparent Google account` OAuth credential is reconnected. Executive report live in GHL. Report rollups, attribution bridge, QA/alerts, and executive summary API all running.
