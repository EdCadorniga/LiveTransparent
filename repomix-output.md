...
- Architecture: the dashboard is a static HTML and JavaScript SPA at reports.livetransparent.com. It calls the n8n executive summary proxy at `/api/report/executive/summary` and a separate campaign summary webhook at `/webhook/lt-report-campaign-channel-summary`; both render client-side. The public host serves build `2026-07-31-v11-campaign-breakdown` with the current campaign table and channel filters.
...
- Campaign response shape: the campaign summary returns `window` and `campaignChannelBreakdown` rows containing `channel`, `campaign`, SMS metrics, email metrics and rates, LinkedIn invite/accept/DM/reply metrics, and Vapi outcome metrics. DAN attribution uses release logs; Emerald uses bucket/enrollment data; Partnership uses `partnership_release_log`; SMS uses `campaign_key`; LinkedIn uses activity events joined to Brand/Dispensary source pools with `campaign_type`/`source_key = 'partnership'` routing to a populated `Partnership LinkedIn` row; and Vapi uses queue campaign IDs. Dynamic source campaigns remain visible as additional rows.
...
- Reporting implementation handoff: native GHL report `6a67dce4a51a4360c60963a3` plus the read-only Executive Report at `reports/embed/executive/index.html`. The public report host now serves build `2026-07-31-v11-campaign-breakdown`.
...
- **SimpleTexting next step**: resolve the provider-side HTTP 409 through SimpleTexting support/account configuration or an approved provider-console test. Then confirm a real provider message ID, correct `report_sms_sent.provider_response`, and campaign advancement only after provider success. Keep the runner and backfill active at bounded volume; do not force additional live sends while 409 persists.
...
- **Add campaign-level reporting to the native GHL report and Executive Report.** The live endpoint now returns named channel/campaign rows, including zero-safe catalog rows for `General outbound`, `Partnership emails`, `xyz`, and `abc`, plus a populated `Partnership LinkedIn` row driven by the `connection_request_sent` activity ledger; the public Executive Report build `2026-07-31-v11-campaign-breakdown` renders All, Email, LinkedIn, SMS, and VAPI filters plus LinkedIn Invites/Accepted columns. Native GHL widget configuration remains a separate operational view; LinkedIn DM/reply/accepted event instrumentation is still pending for nonzero campaign activity. Do not stop at date-level totals. Define a canonical campaign dimension and show each campaign separately for the selected period and immediately preceding equal-length period.
...
Updated: 2026-07-31 (partnership state seeding and dry-run verification, GHL PIT REST verification, campaign-level reporting attribution, live host verification, native GHL report access check, plus prior SimpleTexting, voice, classification, ownership, and PIT-rotation work)
...
1. ~~**Deploy and verify the Executive Report**~~ **Done 2026-07-31**: the public host serves build `2026-07-31-v11-campaign-breakdown`; campaign/channel rows, selected-period controls, prior-period comparison, LinkedIn Invites/Accepted columns, and the HTTP 200 campaign endpoint were verified. Coolify's Git webhook did not deploy `main`, so the verified image was rebuilt from `main` and applied to the managed VPS container; the generated compose file was backed up before replacement.
...
- **Campaign-level reporting dimensions**: `LT - Report Campaign Channel Summary` (`MvPLbUAN9IIQikxb`, active version `14caff0f-fa99-4bab-9867-792ef8836de6`) returns named campaign/channel rows. DAN uses release-log campaign fields, Emerald uses bucket/enrollment data, SMS uses `SimpleTexting_Campaign_Event_Log.campaign_key`, LinkedIn uses `linkedin_activity_events` joined to `emerging_pool_contacts.source_list`, and Vapi uses queue campaign IDs. `General outbound`, `Partnership emails`, `xyz`, and `abc` are present as catalog rows. The public report build `2026-07-31-v11-campaign-breakdown` has All, Email, LinkedIn, SMS, and VAPI campaign filters, plus LinkedIn Invites/Accepted columns. Emerald event attribution still has an `Emerald - attributed` fallback. LinkedIn activity is populated from `linkedin_activity_events`: the `Partnership LinkedIn` row shows 10 `connection_request_sent` events for the verified 2026-07-31 run; dispatcher instrumentation writes invite and diagnostic events, and a reconciliation backfill covers requested/connected state rows. DM/reply/accepted event instrumentation remains a follow-up.
