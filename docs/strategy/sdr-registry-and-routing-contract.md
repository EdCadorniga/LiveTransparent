...
- Live `Jason Followup Emails and SMS` (`f6b44e34-779e-4959-b41d-b05641f134e7`) is published at version 39 (audited 2026-07-30). All 7 Send Email actions use owner-driven sender fields: `From Name = {{opportunity.owner}} from Transparent eCom` and `From Email = {{user.email}}`. Workflow-level defaults are `Jason from Transparent eCom` / `jason@livetransparent.com`. The workflow triggers on Sales Outreach stages: New (`3529dd3d`), Attempting Contact 1st Attempt (`b97e42b1`), 2nd Attempt (`c46c3be3`), 3rd Attempt (`c8b7a450`), Engaged (`9ced8010`).
- The six templates in `Jason Follow Up Emails` retain `Jason from Transparent eCom <jason@livetransparent.com>` as literal fallback metadata. One template (`69e0dcad8ffabf47b4d987c5`, "Cannabis Ads: Next Steps") is reused by 2 of the 7 email actions.
- **Marc routing path is untested in production**: as of 2026-07-30, zero Marc-owned (`sqGx5rp3oAUG610NXyjU`) opportunities exist in any of the 5 trigger stages. All Marc-owned opportunities are in the Qualified stage and have not yet entered a stage that fires this workflow.
- The template API rejects merge fields in `fromEmail`, so owner routing is implemented at the workflow-action level (not template metadata). The three-layer defense is: action merge fields → template fallback → workflow default.
...
- SimpleTexting and some social/Vapi paths still contain channel-specific identity or compatibility literals and are not yet registry-driven. The 14 SMS follow-ups in this workflow use legacy compatibility aliases and are not owner-routed.
- The published `LT - Opportunity Owner Alignment` workflow (`b26326a5-77af-4df8-8d86-3f636e73afe0`, version 7) synchronizes contact owner, opportunity owner, custom opportunity owner, and routing audit fields for Jason (`yU85G6kfhtW4vUtx3QE6`) and Marc (`sqGx5rp3oAUG610NXyjU`). This is owner alignment, not a generic qualification allocator.
...
The existing `LT - Micro - Email Open Counter + Assignment to Jason` workflow is not a complete implementation of this contract: its published branches hardcode opportunity custom-owner values and do not update the native contact owner. The `Email Open 3x - Assigned Kevin` tag is a campaign guard, not an authoritative owner field, and should not be used as the owner source of truth.
