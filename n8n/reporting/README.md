...
- Current live state: GHL Leads Ingest is verified, but GHL Sales Ingest remains blocked by HTTP `401` and unsafe Postgres v2.6 writes; use the post-recovery baseline in `docs/handoff/2026-08-12-report-recovery.md`. The report API is responding and the public Executive Report serves build `2026-08-12-v23-mobile-overflow` with resolved GHL stage labels and responsive table containment.
