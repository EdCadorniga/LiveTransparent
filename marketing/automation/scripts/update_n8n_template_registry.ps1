...
Write-Host "Merging new jason_sms templates into templateRegistryJson"

$newEntries = @{
    john_sms1 = @{ name = 'Jason SMS 1 - Initial Outreach'; message = "Hi this is Jason, just gave you a call. Saw you were interested in learning about ads for regulated industries on social/search.\n\nWe run ads for Mood, Cookies, and more! Interested in learning how?" }
    john_sms2 = @{ name = 'Jason SMS 2 - Locked Out?'; message = "Hey {{first_name}}! Are you locked out of ads, or just avoiding them because of the horror stories?\n\nI can show you how top regulated-industry brands are doing it in 10 mins." }
    john_sms3 = @{ name = 'Jason SMS 3 - Scale Year'; message = "Hi {{first_name}} this could be the year you scale your brand on social/search! Interested in how we do it for Mood, Cookies, and more?" }
    john_sms4 = @{ name = 'Jason SMS 4 - Last Follow-up'; message = "Hi {{first_name}}—last follow-up on ads for regulated industries. Is it timing, or is there a better contact?" }
    john_sms5 = @{ name = 'Jason SMS 5 - Engaged Follow-up'; message = "Good chatting about ads for regulated industries earlier—based on what you shared, this looks like a strong fit.\n\nWe're onboarding a few brands this month—grab a time here: {{trigger_link.nqLFBlEsdm7qccr8Yyog}}" }
}
