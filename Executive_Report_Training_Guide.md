...
- KPI Row: The six cards at the top summarize the selected date window: Sessions, Contacts, Opportunities, Meetings, Closed Won, and Revenue. Sessions is traffic volume. Contacts is CRM volume. It is normal for these to differ because a contact is not always created by a form.
...
- Capture Gaps: This is an absolute-volume panel. It shows Sessions, Forms, Contacts, Opportunities, Meetings, and Closed Won side by side. Do not read it as a perfectly linear funnel because contacts can arrive from routing, manual CRM entry, imports, and follow-up as well as forms.
- Sales and Pipeline: This section provides the company-wide pipeline summary. It covers pipeline counts, stage movement, velocity, and sales quality. Use it when discussing pipeline health, not acquisition quality.
...
- Funnel basis: the primary funnel rates now use Users as the denominator where possible. This means the dashboard is treating unique visitors as the main traffic audience, not raw sessions.
...
|---|---|---|
| Sessions | GA4 visits in the selected window. | Traffic volume only. One person can generate multiple sessions. |
...
| Contact -> Opportunity | Contacts that became opportunities in the selected window. | Cohort-safe and not inflated by multiple opportunities per contact. |
...
| Cohort Contacts | New contacts created in the selected window. | This is the denominator for attribution coverage. |
...
- Use Users-based funnel rates for primary interpretation, and use Sessions only as traffic context.
